...
- Обнаруживает дальние цели с задержкой раскрытия (опционально)
- Цвет полоски HP меняется динамически в зависимости от состояния здоровья
...
| `update_ms` | 17 | Частота обновления выбора цели (мс), 0 = каждый кадр |
...
| Параметр | По умолчанию | Описание |
|----------|-------------|----------|
| `jitter_mode` | 0 | Режим: 0 = выкл, 1 = deadzone, 2 = smooth 2D, 3 = smooth 3D |
| `jitter_deadzone_px` | 2 | Мёртвая зона (px) для deadzone режима |
| `jitter_tau_ms` | 120 | Время сглаживания (мс) для EMA режимов |
| `jitter_max_step_px` | 0 | Максимальный шаг (px) за кадр (0 = без ограничений) |

### Интерполяция UI

| Параметр | По умолчанию | Описание |
|----------|-------------|----------|
| `lerp_factor` | 0.3 | Скорость интерполяции к целевой позиции за кадр (0.01-1.0, меньше = плавнее) |
...
**Цвет HP-бара:** Полоска HP меняет цвет динамически в зависимости от состояния здоровья:
- 100% здоровья: зелёный
- 1% здоровья: красный
- Между 1-100%: плавный градиент от красного к зелёному

### Задержка обнаружения

| Параметр | По умолчанию | Описание |
|----------|-------------|----------|
| `detect_enable` | false | Включить задержанное обнаружение для дальних целей |
| `detect_instant_dist` | 50 | Дистанция мгновенного обнаружения (м) - цели в этом радиусе показывают полную информацию сразу |
| `detect_delay_sec` | 2 | Задержка (сек) перед показом полной информации для дальних целей |

**Анимация обнаружения:** В период задержки показывается текст "Обнаружение..." с анимированной сменой цвета через оттенки серого (тёмный → светлый → тёмный).
...
- Интерполяция UI предотвращает телепорты маркера при резких изменениях позиции (например, анимации костей, телепортация)
- HP-бар использует линейную интерполяцию для цветовых переходов на основе процента здоровья
...
### Версия 1.1+
- Добавлена интерполяция UI для предотвращения телепортов маркера
- Добавлен динамический цвет HP-бара (красный → зелёный в зависимости от здоровья)
- Добавлена функция задержки обнаружения с анимацией "Обнаружение..."
- Исправлено смещение HP-бара, влияющее на все элементы (бар, тень, фон)
- Улучшен anti-jitter с настраиваемыми параметрами сглаживания
